...
let sender1: Viewer;
let firstMessageExternalID: string;
let firstReplyToMessageExternalID: string;
...
async function markAsRead(viewer: Viewer, messageID: UUID) {
  const result = await executeGraphQLOperation({
    ...
    variables: { messageID, byExternalID: true },
    viewer,
  });
...
describe('Test marking message as read', () => {
  beforeAll(async () => {
    ...
    const m = await MessageEntity.findByPk(firstMessage1ID);
    firstMessageExternalID = m!.externalID;

    const { messageID: firstReply } = await addMessageViaGraphQL(sender1, {
      threadID: thread1ID,
    });
    const rm = await MessageEntity.findByPk(firstReply);
    firstReplyToMessageExternalID = rm!.externalID;
  ...
  test('mark reply as read marks just that notif', async () => {
    ...
    await markAsRead(recipient1, firstReplyToMessageExternalID);
    ...
    await markAsRead(sender1, firstMessageExternalID);
